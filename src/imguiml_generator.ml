...
let rec pp_ctypes_type ~in_bindings ~lift ~mod_name fmt = function
  ...
  | TComp ({ cname; _ }, _) ->
      ...
      else if module_name = !current_struct then Format.fprintf fmt "@[<hov>t@]"
  ...
  | Some args ->
      Format.pp_print_list ~pp_sep:pp_ctypes_arrow
        (fun fmt (name, typ, _) ->
          ...
          | _ -> pp_ctypes_type ~in_bindings ~lift ~mod_name fmt typ)
        fmt args
