...
module Bindings (F : Ctypes.FOREIGN) = struct
  ...
  module OpenGL3 = struct
    ...
    let new_frame =
      foreign "ImGui_ImplOpenGL3_NewFrame" (void @-> returning void)
  ...
  module Glfw = struct
    ...
    let init_for_opengl =
      foreign "ImGui_ImplGlfw_InitForOpenGL"
        (ptr window @-> bool @-> returning bool)

    let shutdown = foreign "ImGui_ImplGlfw_Shutdown" (void @-> returning void)
    let new_frame = foreign "ImGui_ImplGlfw_NewFrame" (void @-> returning void)
  end
end
